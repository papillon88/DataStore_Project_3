...
{	
	...
	private ServerCommand dispatchCommand(InputStream inputStream) throws ServerException
	{
		...
		try
		{
			commandName = StreamUtil.readLine(inputStream);
		}
		catch (IOException e)
		{
			logger.error("ERROR: could not proccess command name; returning INVALID command");
			e.printStackTrace();
			commandName = "invalid";
		}
		
		ServerCommand serverCommand = new InvalidCommand();
		
		switch (commandName)
		{
		case "write":
			logger.debug("creating WRITE command...");
			serverCommand = new WriteCommand();
			break;
		case "read":
			logger.debug("creating READ command...");
			serverCommand = new ReadCommand();
			break;
		case "delete":
			logger.debug("creating DELETE command...");
			serverCommand = new DeleteCommand();
			break;
		case "directory":
			logger.debug("creating DIRECTORY command...");
			serverCommand = new DirectoryCommand();
			break;
		case "invalid":
			logger.debug("creating INVALID command...");
			break;
		default:
			break;
		}
